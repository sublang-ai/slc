...
For DEV planning outcomes, the runner calls `profile.select(semantic, { finalText })` with the exact Analyst text just returned by the fixture actor.
Generated DEV profiles may project that value into actually advertised semantic fields such as `planningResult` or `question`; the governed reply check still rejects fields the runtime did not advertise.
