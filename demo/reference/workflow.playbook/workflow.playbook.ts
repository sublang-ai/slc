// Linked playbook runtime for the Two-Agent Code-and-Review Workflow.
//
...
/**
 * The FSM declares no required machine-input field, so the only option is the
...
const spec: XStatePlaybookRuntimeSpecV3<PlaybookRuntimeOptions> = {
  ...
  machineInput: () => ({}),
  // `START` is the FSM's only Boss entry event and carries no payload; it
  // disappears under type erasure, so the classifier needs it declared here.
  // `BOSS_INTERRUPT` (with its closed target set) and `BOSS_REPLY` are derived
  // by the factory from the machine itself.
  bossEvents: [{ type: 'START' }],
  ...
  // Loop and argument-round progress only: the review findings, judgments, and
  // rebuttals are player-authored text and stay private.
  controlContextFields: ['reviewLoops', 'judgmentCount'],
};
