...
When Boss gives the input task and work has not begun, Captain shall ensure the current directory is the root of its own Git repository:

> Check whether the current directory is the root of its own Git repository.
> If `.git` is absent there, initialize a repository there.
