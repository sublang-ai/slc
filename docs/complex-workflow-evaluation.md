...
Prepared C4 and v6/scaffold-v3 inputs extend those checks without accepting a new performance or full-workflow result. C4 uses production commit `c021339` from a private archive build: relative to immutable C3 it changes 37 `dist/` files, while dependencies, pipelines, pins, and scripts remain unchanged. The 2,400-second stall watchdog default from external commit `8db05c6` is included among these input differences and carries no optimization claim. The v6 link pair is frozen under upstream commit `642875f` with boundary proof evidence, and the scaffold-v3 fixed-GEARS readiness confirms the same 8,432-byte VucDX9 GEARS source, identical current initializer and guide files across both arms, and only `playbook/gears2fsm.md` differing. Independent live comparison results remain pending.
