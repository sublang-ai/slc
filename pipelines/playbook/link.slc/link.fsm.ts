...
const LINK_10_PROMPT: string[] = [
  ...
  "Treat adjudicator failures as control-plane errors: propagate them by throwing out of `handleBossInput` after attempting cleanup (the host adapter surfaces the throw on its control-plane channel — e.g. cligent surfaces such throws as `runtime_error`).",
  ...
  "Player prompts and adjudicator JSON ride the host's own record channels when the host has them (e.g. cligent's `captain_*` / `player_*`); do not duplicate them into `emitTelemetry`.",
 ...
 */
function bossInterrupts(ids: readonly string[], actions?: LinkActionName): GuardedArm[] {
  return ids.map((id) => ({
...
export const linkMachine = setup({
  ...
  actions: {
    ...
    startLink: assign({
      bossRequest: ({ event }) => (event.type === 'START_LINK' ? event.request : undefined),
  ...
  states: {
    ...
    linking: {
      id: 'linking',
      description: 'Captain carries out the FSM-to-Runtime link transformation specified by LINK-10.',
      invoke: {
        src: 'captain',
        input: ({ context }): CaptainInput => ({
          ...
          prompt: linkPrompt,
          result: {
            completed: 'Captain has carried out the transformation and emitted the PlaybookRuntime module.',
        ...
        BOSS_REPLY: [
          {
            ...
            guard: ({ event }) => event.type === 'BOSS_REPLY' && event.answer.trim() === '',
            target: 'failed',
            actions: 'clearBossReplyContext',
          },
    ...
    failed: {
      id: 'failed',
      description: 'A captain invocation failed; recoverable via a Boss interrupt or a fresh request.',
      on: {
        START_LINK: { target: 'linking', actions: 'startLink' },
      },
    },
      ...
      id: 'done',
      type: 'final',
      description: 'The link transformation completed and the PlaybookRuntime module was emitted.',
    },
  },
  on: {
