...
const machineSetup = setup({
  ...
  actions: {
    ...
    setPendingBossQuestion: assign(
      (
        { event },
        params: { resumeStateId: string; sourceItem: string; player: string },
      ) => ({
        pendingBossQuestion: {
...
export const gears2fsmMachine = machineSetup.createMachine({
  ...
  states: {
    ...
    ready: {
      id: 'ready',
      description:
        'Quiescent idle hub; waits for a Boss entry event before any player call.',
      on: {
    ...
    transform: {
      id: 'transform',
      description:
        'Captain carries out the GEARS-to-FSM transformation specified by G2F-1.',
      invoke: {
        src: 'captain',
    ...
    failed: {
      id: 'failed',
      description:
        'A captain invocation or Boss reply failed; recoverable via a Boss directive.',
      on: {
        START: { target: '#transform', actions: 'applyBossEntry' },
      },
    },
