...
const DEFAULT_PLAYER_BINDING: Readonly<Record<string, string>> = { Captain: 'captain' };
...
const QUIESCENT_STATES: readonly string[] = ['ready', 'awaitBossReply', 'failed'];
...
const BOSS_EVENTS: ReadonlyArray<{
    ...
    },
    {
        ...
        payload: { answer: 'required string — the Boss answer to the pending question' },
    },
];
 ...
 */
function resolvePlayerId(input: CaptainInput, binding: Record<string, string>): string {
...
function buildClassifierPrompt(
    ...
    if (ctx.pendingQuestion) {
        lines.push('A player has paused and is waiting for Boss to answer this question:');
...
function parseClassification(raw: string): TransformEvent | null {
    ...
    if (type === 'START') {
        const req = obj.request as { source?: unknown; target?: unknown } | undefined;
        if (req && typeof req.source === 'string' && typeof req.target === 'string') {
            return { type: 'START', request: { source: req.source, target: req.target } };
        }
        return { type: 'START' };
    }
    if (type === 'BOSS_INTERRUPT') {
        return typeof obj.targetId === 'string' ? { type: 'BOSS_INTERRUPT', targetId: obj.targetId } : null;
    }
    if (type === 'BOSS_REPLY') {
        return typeof obj.answer === 'string' ? { type: 'BOSS_REPLY', answer: obj.answer } : null;
    }
    return null;
}
...
function combineSignals(a: AbortSignal | undefined, b: AbortSignal | undefined): AbortSignal {
 ...
 */
export default function createPlaybookRuntime(options: PlaybookRuntimeOptions): PlaybookRuntime {
    const binding: Record<string, string> = { ...DEFAULT_PLAYER_BINDING, ...(options.playerBinding ?? {}) };
    const fsmInput: TransformInput = { players: binding, request: options.request };
    ...
    const enqueue = (fn: () => Promise<void>): void => {
        emissionChain = emissionChain.then(() => fn().catch(() => undefined));
    };
    const flush = (): Promise<void> => emissionChain;

    const requirePorts = (): PlaybookPorts => {
        if (!ports) throw new Error('gears2fsm runtime: init(ports) must be called before use');
        return ports;
    };

    // The runtime-owned Captain actor: composes the prompt, calls the player,
    // and adjudicates the result into an FSM guard.
    const captain = fromPromise<CaptainOutput, CaptainInput>(async ({ input, signal }) => {
        ...
        try {
            return parseAdjudication(await p.callJudge(buildAdjudicatorPrompt(input, result.finalText ?? ''), combined), input);
        } catch (err) {
            // Abort during adjudication is a natural rejection (routes to `failed`),
            // not a control-plane adjudicator failure.
            if (!combined.aborted) adjudicatorError = err;
            throw err;
        }
    });
    ...
    const inspect = (event: InspectionEvent): void => {
        ...
        if (!p) return;
        enqueue(() =>
            p.emitTelemetry({ topic: TELEMETRY_TOPIC, payload: { from: from ?? null, to, event: eventType } }),
        );
        enqueue(() => p.emitStatus(`FSM ${from ?? '(init)'} -> ${to} [${eventType}]`));
        if (to === 'failed') {
    ...
    const isQuiescent = (snapshot: SnapshotFrom<typeof gears2fsmMachine>): boolean =>
        snapshot.status === 'done' || !CAPTAIN_STATES.includes(String(snapshot.value));
    ...
    const driveToQuiescence = (): Promise<void> =>
        new Promise<void>((resolve) => {
            ...
            sub = live.subscribe(check);
            check(live.getSnapshot());
            if (settled && sub) sub.unsubscribe();
        });

    const classify = async (text: string, signal: AbortSignal): Promise<TransformEvent | null> => {
    ...
    return {
        ...
        async handleBossInput(turn: { text: string; signal: AbortSignal }): Promise<void> {
            requirePorts();
            if (!actor) throw new Error('gears2fsm runtime: init(ports) must be called before handleBossInput');
