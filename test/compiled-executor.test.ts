...
describe('createCompiledExecutor (PHEXEC-26)', () => {
  ...
  it('appends the update context to transformation-performing prompts (INCR-15)', async () => {
    const target = join(root, 'out.ts');
    ...
    const result = await executor.run(
      {
        ...
        target,
        update: {
          priorInput: join(root, '.slc/builds/1/source'),
          diff: '@@ -1,1 +1,1 @@\n-old\n+new',
        },
      },
      new AbortController().signal,
    ...
    expect(prompts[0]).toContain(join(root, '.slc/builds/1/source'));
