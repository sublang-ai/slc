...
| Reviewed artifacts | Require all twelve generated bundle tests in the full suite, the three independent artifact reviews with each FSM's concurrent-role export, own artifact-schema evidence, canonical linked module, actor-applicable composers, and fail-closed schema, conformance, composition, and coverage findings, pin regeneration, and a byte-identical pin-index diff [[continuous-integration-2](#continuous-integration-2)]. |
