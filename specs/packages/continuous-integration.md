...
Where a commit is pushed or proposed by pull request, when repository continuous integration runs, the workflow shall exercise the English demo reference check, validate the publishable tarball in an empty consumer project [[release-10](release.md#release-10)] under the documented artifact-local dependency resolution contract [[release-11](release.md#release-11)], and verify that the release workflow publishes through trusted OIDC alone with no static registry credential [[release-8](release.md#release-8)].
When a demo reference check runs, it shall drive that language's committed reference over its own runtime boundary and shall also offer the same committed registry entry to the installed Playbook host's own registry validation — the host's deterministic catalog listing of a scratch configuration that enables the entry with every declared role bound to one placeholder player, resolved under scratch Spex-root, session-state, and superseded-configuration homes and with engine provisioning disabled, so the stage spends no model call, needs no adapter SDK, and writes nothing beside the committed artifacts or under the invoking user's home — because a manifest field only the host reads is invisible to a check that merely constructs the runtime:

| Reference's declared role ids | Required outcome |
| --- | --- |
| Every declared role id is a canonical lowercase local role id, as the English reference's are. | Require the host to accept the configuration and list the entry under its own id. |
| A declared role id is not, as the Chinese reference's are not. | Require the host to refuse the configuration naming that role key, pinning the known binding gap rather than passing it in silence. |
...
| Release surfaces | Require the English reference checker — including the host registry-validation stage that lists a canonical-role reference and requires a non-canonical one's refusal to name the role key — installed-package smoke, and trusted-publication workflow audit [[continuous-integration-5](#continuous-integration-5)]. |
