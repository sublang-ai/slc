...
Where a faked Coder or Reviewer transport yields an initial event and then stalls under a short configured stall timeout, when the call runs through the phase transport or review wrapper, the slc command shall abort the stalled call once the timeout elapses, preserve a failure diagnostic carrying the inactivity duration, and make no automatic transport retry of that call [[phase-execution-36](#phase-execution-36)], [[phase-execution-12](#phase-execution-12)], with a stalled Reviewer call retried once by the review wrapper before its preserved diagnostic fails the performing call closed [[phase-execution-46](#phase-execution-46)]; whereas where the aborted transport still yields a successful terminal event within the post-abort drain, the slc command shall report that success rather than a stall [[phase-execution-36](#phase-execution-36)].
