...
| Factory argument | One plain accessor-free object with exactly `configuredOptions` and `hostCapabilities`; `configuredOptions` contains no host capability and is the exact empty plain object, offered first as the roleless baseline, or — only when the factory rejects that empty construction — exactly `{ definition }` in one further construction, so the factory's own options validation decides which of the two it binds; a factory rejecting both fails the phase with both rejections named. |
