...
| --- | --- |
| Factory | A callable carrying an enumerable, non-writable, non-configurable own `compat` data property whose frozen exact own-data value is `{ artifactSchema: 3, runtimeAbi: 1 }`; a missing, accessor-backed, mutable, or different declaration fails before construction. |
...
| `hostCapabilities` | One live plain accessor-free object with exactly enumerable own data properties `authority`, `repository`, and `effectLedger`; no member enters configured options, machine input, context, session snapshots, or diagnostics. |
| `authority` | An accessor-free plain record with exactly enumerable own data properties `playbookId`, `artifactSchema`, `cwd`, `sessionId`, `leaseOwnerToken`, `canonicalWorktree`, `requiredRoleIds`, and `concurrentRoleSets`; those values are respectively the selected phase id, literal `3`, absolute invocation working directory, causal root's globally unique id, run-unique nonempty token, exact plain accessor-free `{ worktree, gitDir }` canonical identity of the Git worktree containing that directory, and two empty arrays; inability to resolve that worktree and Git directory fails before construction. |
| `repository` | An accessor-free plain live object with exactly enumerable own data properties `identity`, `observe`, `acquire`, `runExclusive`, `runCohort`, and `runDeferred`; `identity` is an exact plain accessor-free `{ worktree, gitDir }` copy equal to `authority.canonicalWorktree`, and each other property is callable and rejects with the same deterministic unsupported phase-host repository error without invoking a supplied operation. |
| `effectLedger` | An accessor-free plain live object with exactly enumerable own data properties `snapshot` and `writeAhead`; synchronous `snapshot()` returns a detached empty ledger `{ schemaVersion: 1, revision: 0, boundaries: [], logicalOperations: [] }`, and `writeAhead` rejects with a deterministic unsupported phase-host effect error without mutation. |
| Unsupported construction or capability use | An artifact that declares a bespoke schema-3 implementation, lacks the required shared-factory compatibility, rejects empty configured options or empty role/cohort authority, calls a repository operation or ledger write, exposes only the options-only session-Captain wrapper, or otherwise rejects this exact construction fails the compiled phase as an error without agent transport, profile inference, or initialization retry. |
