...
| `-o` is absent; the entry path does not alias the raw source; and an exact-10.0.0 schema-3 artifact under a valid shared or bespoke declaration has an invalid role or concurrent-set declaration | Fail entry emission for each removed alias form, invalid or reserved canonical local id, duplicate or canonical collision, undeclared or repeated concurrent role, omitted or multiply represented simultaneously active region, same member set repeated in any order, and source/FSM mismatch [[self-hosting-15](#self-hosting-15)]. |
