...
Where the dependency manifest selects `@sublang/playbook@^4.0.0` and `@sublang/cligent@^0.18.0` and the lock resolves immutable Playbook 4.0.0 and Cligent 0.18.0, when the SLC repository maintains the current shared definition set, the adopted set shall contain the released `text2gears`, `gears2fsm`, `link`, and `optimize` normative content with SLC's explicit `## Pin Inputs` retained, the three reviewed meta-phase artifact bundles retained because every recorded definition, link-target, and engine input is byte-identical to the preceding reviewed set, and every corresponding pin regenerated with exact `@sublang/playbook@4.0.0` link-target provenance and the shared-engine `@sublang/playbook` runtime dependency recorded beside `xstate`; the Playbook dependency manifest and lock, all four definitions, all three bundles, and `slc.pins.json` shall be accepted only as one current set produced from a clean registry install without a sibling checkout ([DR-011](../decisions/011-playbook-1-0-captain-contract-adoption.md), [DR-017](../decisions/017-playbook-2-0-thin-runtime-adoption.md), [DR-018](../decisions/018-playbook-3-1-adoption.md), [DR-020](../decisions/020-playbook-4-0-adoption.md), [DR-025](../decisions/025-defer-playbook-10-activation.md), [[pinning-15](pinning.md#pinning-15)]).
