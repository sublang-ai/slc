...
| --- | --- |
| Exact Playbook 10.0.0 schema 3 with a valid shared or bespoke implementation, no `Roles` declaration or valid Roles, and valid matching concurrent-role declarations | A Playbook 10 host enables the entry at `playbooks.<basename>.from`, binds every local role at `playbooks.<basename>.roles`, and performs the compiled workflow from the sole Boss input `/<basename> <task>` with no hand-written runtime wiring. |
