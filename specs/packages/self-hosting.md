...
| --- | --- |
| Exact Playbook 10.0.0 schema 3 with a valid shared or bespoke implementation, no `Roles` declaration or valid Roles, and valid matching concurrent-role declarations | A Playbook 10 host enables the entry at `playbooks.<basename>.from`, declares stable player ids in the host's top-level `players`, maps every local role to one of those ids at `playbooks.<basename>.roles`, maps roles that occur together in any concurrent set to distinct player ids, and performs the compiled workflow from the sole Boss input `/<basename> <task>` with no hand-written runtime wiring. |
