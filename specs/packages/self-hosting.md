...
The schema-3 definition and artifact set adopted under [DR-024](../decisions/024-playbook-10-schema-3-adoption.md) and its completed activation in [DR-027](../decisions/027-complete-playbook-10-activation.md) is current at whichever Playbook release the dependency lock resolves, with schema-1 support retained only for historical artifacts; releases are adopted routinely under [DR-028](../decisions/028-contract-based-adoption-without-recompilation.md), which rebuilt the three meta-phase bundles once as compiled-execution control shells and retains each thereafter by verified equivalence.
...
Where the dependency manifest declares `@sublang/playbook` and `@sublang/cligent` each as the caret range of the exact immutable release the lock resolves — with SLC’s direct Cligent requirement and Playbook’s transitive requirement each resolved by the lock — when the SLC repository maintains the current shared definition set, the adopted set shall contain the released `text2gears`, `gears2fsm`, `link`, `optimize`, and `prefix` definitions and their published sidecar-declared local inputs byte-identical to that installed release with those local inputs included in their semantic closures and their pin-input declarations carried by the SLC-owned `slc.pin-inputs.json` sidecar rather than inline `## Pin Inputs` sections, the three reviewed meta-phase artifact bundles as shared-factory schema-3 artifacts — each retained across an adoption when its definition declares a `## Compiled execution` section preserved verbatim in its GEARS [[verification-23](verification.md#verification-23)] and it passes its generated verification and the independent review against the installed engine, and rebuilt through interpreted real-agent runs only when its definition declares no such section or such a check fails — and every corresponding pin regenerated with that release's exact link-target provenance and the shared-engine `@sublang/playbook` runtime dependency recorded beside `xstate`; both demo reference sets shall be retained under the same rule with their checkers re-run against the installed engine; and the Playbook dependency manifest and lock, all five definitions and their local inputs, all three bundles, and `slc.pins.json` shall be accepted only as one current set produced from a clean registry install without a sibling checkout, so that adopting a Playbook release consists of raising the dependency, re-synchronizing the definitions byte-identically, regenerating the pins, and running the verification chain, with a decision record only where the engine's declared contract or a definition's compiled-execution section changes ([DR-011](../decisions/011-playbook-1-0-captain-contract-adoption.md), [DR-018](../decisions/018-playbook-3-1-adoption.md), [DR-024](../decisions/024-playbook-10-schema-3-adoption.md), [DR-026](../decisions/026-slc-owned-pin-input-declarations.md), [DR-027](../decisions/027-complete-playbook-10-activation.md), [DR-028](../decisions/028-contract-based-adoption-without-recompilation.md), [[pinning-15](pinning.md#pinning-15)]).
...
| `-o` is absent; the entry path does not alias the raw source; and no two distinct declared role names collide case-insensitively | Write `<cwd>/<basename>.ts` default-exporting the source generation's registry entry — a historical `Players` fixture's schema-1 entry, or a `Roles` fixture's schema-3 entry advertising `artifactSchema: 3`, a `runtimeProfile` whose own `kind` is `shared-factory` and whose `compat` is the linked factory's own compatibility record by identity, and the declared concurrent-role sets and composing the `{ configuredOptions, hostCapabilities }` factory argument — whose `id` is `<basename>`, importing the linked module by its source-only relative specifier, and whose `createRuntime` hands the host's `callPlayer` port only ids the entry declares: a display-named `Roles` fixture's `requiredRoleIds` are the canonical lowercase role ids its FSM's delegated states name and reach the port unchanged, while a `Players` fixture's `requiredRoleIds` are the verbatim declared names, a lowercased declared id maps back to its declared form, an unknown id passes through, and the runtime's optional capabilities keep their presence [[self-hosting-15](#self-hosting-15)]; the entry performs the compiled workflow with no hand-written wiring through its generation's host [[self-hosting-14](#self-hosting-14)]. |
...
Where a clean registry install resolves the exact `@sublang/playbook` and `@sublang/cligent` releases the lock names and the repository vendors the adopted shared definitions, when adoption acceptance runs, the reserved `slc` and `playbook` references shall both resolve to the vendored `text2gears`, `gears2fsm`, `link`, `optimize`, and `prefix` set and its published sidecar-declared local inputs byte-identical to that installed release with every such input in the corresponding semantic closure and pin-input declarations carried by the SLC-owned sidecar, all three reviewed artifact bundles shall pass their generated verification and independent reviews — compiled-execution fidelity included wherever a definition declares the section — as shared-factory schema-3 artifacts against the installed engine, every corresponding pin shall be current with that release's exact link-target provenance and the recorded shared-engine runtime dependency, both demo reference sets shall pass their checkers against the installed engine, and changing the Playbook dependency, any definition, bundle, or pin away from that one set shall make acceptance fail rather than pass a mixed version [[self-hosting-11](#self-hosting-11)].
