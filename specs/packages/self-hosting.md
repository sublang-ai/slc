...
Where the dependency manifest declares `@sublang/playbook` and `@sublang/cligent` each as the caret range of the exact immutable release the lock resolves — the Cligent floor being the one that installed Playbook release requires — when the SLC repository maintains the current shared definition set, the adopted set shall contain the released `text2gears`, `gears2fsm`, `link`, and `optimize` definitions byte-identical to that installed release with their pin-input declarations carried by the SLC-owned `slc.pin-inputs.json` sidecar rather than inline `## Pin Inputs` sections, the three reviewed meta-phase artifact bundles as shared-factory schema-3 artifacts — each retained across an adoption when its definition declares a `## Compiled execution` section preserved verbatim in its GEARS [[verification-23](verification.md#verification-23)] and it passes its generated verification and the independent review against the installed engine, and rebuilt through interpreted real-agent runs only when its definition declares no such section or such a check fails — and every corresponding pin regenerated with that release's exact link-target provenance and the shared-engine `@sublang/playbook` runtime dependency recorded beside `xstate`; both demo reference sets shall be retained under the same rule with their checkers re-run against the installed engine; and the Playbook dependency manifest and lock, all four definitions, all three bundles, and `slc.pins.json` shall be accepted only as one current set produced from a clean registry install without a sibling checkout, so that adopting a Playbook release consists of raising the dependency, re-synchronizing the definitions byte-identically, regenerating the pins, and running the verification chain, with a decision record only where the engine's declared contract or a definition's compiled-execution section changes ([DR-011](../decisions/011-playbook-1-0-captain-contract-adoption.md), [DR-018](../decisions/018-playbook-3-1-adoption.md), [DR-024](../decisions/024-playbook-10-schema-3-adoption.md), [DR-026](../decisions/026-slc-owned-pin-input-declarations.md), [DR-027](../decisions/027-complete-playbook-10-activation.md), [DR-028](../decisions/028-contract-based-adoption-without-recompilation.md), [[pinning-15](pinning.md#pinning-15)]).
