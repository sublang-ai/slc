...
Where `-o` is absent, the deterministic entry path does not alias the raw source, and the full-link input closure satisfies one of the supported entry contracts below [[self-hosting-15](#self-hosting-15)], when the user runs the `playbook` pipeline on a source without `--link`, the slc command shall additionally produce a runnable entry module `<basename>.ts` beside the artifact directory whose invocation follows that contract ([DR-014](../decisions/014-cwd-output-invocation-defaults-entry-emission.md), [DR-024](../decisions/024-playbook-10-schema-3-adoption.md)):

| Entry contract | Invocation |
| --- | --- |
| Exact Playbook 10.0.0 schema 3 with a valid shared or bespoke implementation, no `Roles` declaration or valid Roles, and valid matching concurrent-role declarations [[verification-1](verification.md#verification-1)] | A Playbook 10 host enables the entry at `playbooks.<basename>.from`, declares stable player ids in the host's top-level `players`, maps every local role to one of those ids at `playbooks.<basename>.roles`, maps roles that occur together in any concurrent set to distinct player ids, and performs the compiled workflow from the sole Boss input `/<basename> <task>` with no hand-written runtime wiring. |
| Exact reviewed Playbook 0.10.0, 1.0.0, 2.0.0, 3.1.0, or 4.0.0 schema 1 with a compat-less factory and either no `Players` declaration or case-insensitively unique Players | The matching complete historical Playbook host closure performs the compiled workflow through `playbook run ./<basename>.ts "<task>"` with no hand-written runtime wiring. |
...
Where a full-link run of the `playbook` pipeline has produced its linked artifact, when the slc command settles deterministic outputs, the command shall apply the entry-module outcome for the applicable case ([DR-014](../decisions/014-cwd-output-invocation-defaults-entry-emission.md), [DR-017](../decisions/017-playbook-2-0-thin-runtime-adoption.md), [DR-024](../decisions/024-playbook-10-schema-3-adoption.md)):
...
| `-o` is absent; the entry path does not alias the raw source; the full-link input closure resolves exact `@sublang/playbook@10.0.0` link-target provenance; the linked artifact is schema 3 and either its factory carries an enumerable, non-writable, non-configurable own `compat` data property whose frozen exact own-data value is `{ artifactSchema: 3, runtimeAbi: 1 }` or its bespoke implementation carries exactly `{ kind: 'bespoke', artifactSchema: 3 }` with no runtime-ABI claim; its GEARS has no `Roles` declaration or a valid one whose canonical lowercase local ids match `[a-z][a-z0-9_-]*`, exclude `captain`, and do not collide; and its concurrent-role declaration names one pairwise-distinct role per simultaneously active region, names only declared roles, repeats no set, and matches the FSM's mandatory source-derived export exactly, including `[]` when no group exists [[verification-1](verification.md#verification-1)]. | Deterministically emit `<cwd>/<basename>.ts` — an erasable-TypeScript module importing the linked module via `./<basename>.<pipeline>/<basename>.playbook.ts` and default-exporting a Playbook registry entry with `id` and `command` set to `<basename>`, `artifactSchema` set to `3`, a `runtimeProfile` that truthfully advertises `{ kind: 'shared-factory', compat: createPlaybookRuntime.compat }` for a linked factory carrying its validated immutable compatibility or `{ kind: 'bespoke', artifactSchema: 3 }` for a bespoke linked implementation, `requiredRoleIds` set to the canonical lowercase roles in source order or empty when Roles is absent, `concurrentRoleSets` copied exactly from the FSM's source-derived export, `intent` derived from the normalized source's title and lead line, `validateOptions` a fail-closed validator of the linked configured options not supplied by the Boss turn, and `createRuntime(options, hostCapabilities)` calling the linked default factory exactly once with exactly `{ configuredOptions: options, hostCapabilities }` and returning that runtime directly; emit no player-alias proxy because current host role bindings own concrete player and prompt identity. |
| `-o` is absent; the entry path does not alias the raw source; the full-link input closure resolves exact `@sublang/playbook@10.0.0` link-target provenance; the linked artifact carries one of those valid schema-3 implementation declarations; and its Roles or concurrent-set declaration uses removed alias syntax, derives an invalid or reserved canonical local id, duplicates or canonically collides, names an undeclared or repeated concurrent role, duplicates a concurrent set, or disagrees with the FSM export [[verification-1](verification.md#verification-1)]. | Fail entry-module emission closed with a diagnostic naming the invalid declaration or export mismatch. |
| `-o` is absent; the entry path does not alias the raw source; the full-link input closure resolves exact `@sublang/playbook@0.10.0`, `1.0.0`, `2.0.0`, `3.1.0`, or `4.0.0` link-target provenance; the linked artifact is schema 1; its factory exposes no own `compat`; and declared player ids are unique case-insensitively. | Preserve the historical entry contract: deterministically emit `<cwd>/<basename>.ts` as an erasable-TypeScript module importing `./<basename>.<pipeline>/<basename>.playbook.ts` and default-exporting a registry entry with `id` and `command` set to `<basename>`, `requiredRoleIds` set to the verbatim source-declared Players in source order, `intent` derived from the normalized source's title and lead line, `validateOptions` a fail-closed allowlist of linked options not supplied by the Boss turn, and `createRuntime` calling the linked default factory with validated options whose `cwd` defaults to `process.cwd()`; wrap sessions passed to `init` and optional `restore` so a lowercased runtime player id maps back to its declared form while unknown ids and every other port, runtime member, and optional capability cross unchanged [[phase-execution-30](phase-execution.md#phase-execution-30)]. |
| `-o` is absent; the entry path does not alias the raw source; that exact reviewed historical schema-1 and compat-less contract applies; and two declared player ids collide case-insensitively. | Fail entry-module emission closed with a diagnostic. |
| `-o` is absent; the entry path does not alias the raw source; and any remaining case applies, including absent or unsupported provenance or an artifact schema or factory declaration that conflicts with the contract mapped for its exact reviewed provenance. | Fail entry-module emission closed with a diagnostic naming the unsupported or inconsistent entry contract. |

### Immutable definition adoption

#### self-hosting-11

Where the root manifest selects `@sublang/playbook@^10.0.0` and `@sublang/cligent@^0.23.0` while retaining direct `@sublang/spex@^0.3.0`, and the dependency lock resolves exact Playbook 10.0.0 and Cligent 0.23.0 with every installed Anthropic, Codex, or OpenCode SDK respectively satisfying `>=0.3.219`, `>=0.144.0`, or `>=1.18.12` while an absent optional SDK remains absent, when the SLC repository adopts that release's shared definition set, the adopted set shall contain the released `text2gears`, `gears2fsm`, `link`, and `optimize` normative content with SLC's explicit `## Pin Inputs` retained; `src/normalize.md` synchronized to Roles terminology; all three reviewed meta-phase artifact bundles rebuilt as shared-factory schema-3 linked modules through fresh interpreted real-agent `slc slc` runs and independently verified from those definitions; every corresponding pin regenerated through the build-and-review flow [[pinning-15](pinning.md#pinning-15)] with exact `@sublang/playbook@10.0.0` link-target provenance, the shared-engine `@sublang/playbook` runtime dependency recorded beside `xstate`, the package lock and complete bundle identities, and the unchanged direct Spex 0.3 grammar identities; and the demo manifest, every committed demo reference and entry artifact, both version-bearing demo READMEs, and the configured-registry installed-host acceptance path [[release-17](release.md#release-17)] synchronized to the same release; the root manifest and lock, `composed-v3` execution [[phase-execution-23](phase-execution.md#phase-execution-23)], all four definitions, normalization prompt, all three bundles and their generated tests, `slc.pins.json`, schema-3 entry emission [[self-hosting-15](#self-hosting-15)], complete demo set, and installed-host acceptance consumer shall be accepted only as one current set produced from a clean registry install without a sibling checkout ([DR-011](../decisions/011-playbook-1-0-captain-contract-adoption.md), [DR-018](../decisions/018-playbook-3-1-adoption.md), [DR-024](../decisions/024-playbook-10-schema-3-adoption.md)).
...
| `-o` is absent; the entry path does not alias the raw source; and an exact 10.0.0 schema-3 artifact has a valid shared or bespoke declaration plus absent or valid Roles and valid matching concurrent-role declarations | Write `<cwd>/<basename>.ts` default-exporting a registry entry whose `id` is `<basename>`, `artifactSchema` is `3`, runtime profile exactly advertises the actual shared-factory compatibility or bespoke schema-3 implementation, `requiredRoleIds` are the source roles' canonical lowercase ids or empty, and `concurrentRoleSets` equal the FSM export; import the linked module by its source-only relative specifier; validate configured options; and have `createRuntime(options, hostCapabilities)` call the linked factory exactly once with exactly `{ configuredOptions: options, hostCapabilities }`, return that runtime directly, and expose no player-alias proxy [[self-hosting-15](#self-hosting-15)]; after an isolated Playbook config declares stable ids in top-level `players`, enables the entry at `playbooks.<basename>.from`, binds every required role at `playbooks.<basename>.roles`, and binds roles in each concurrent set to distinct player ids, `playbook run "/<basename> <task>"` performs it from that sole Boss input with no hand-written runtime wiring [[self-hosting-14](#self-hosting-14)]. |
| `-o` is absent; the entry path does not alias the raw source; and an exact-10.0.0 schema-3 artifact under a valid shared or bespoke declaration has an invalid role or concurrent-set declaration | Fail entry emission for each removed alias form, invalid or reserved canonical local id, duplicate or canonical collision, undeclared or repeated concurrent role, same member set repeated in any order, and source/FSM mismatch [[self-hosting-15](#self-hosting-15)]. |
| `-o` is absent; the entry path does not alias the raw source; and an exact 0.10.0, 1.0.0, 2.0.0, 3.1.0, or 4.0.0 schema-1 input closure has a compat-less linked factory and unique declared Players | Write the same canonical import, basename `id` and `command`, derived `intent`, fail-closed option validator, and verbatim source-order `requiredRoleIds`; default an omitted validated `cwd` to `process.cwd()` at options-only linked construction; map lowercased declared player ids back to their declared forms in `init` and optional `restore`; and preserve unknown ids and every other port, member, and optional capability [[self-hosting-15](#self-hosting-15)]; under the matching historical host closure, `playbook run ./<basename>.ts "<task>"` performs it without hand-written runtime wiring [[self-hosting-14](#self-hosting-14)]. |
| `-o` is absent; the entry path does not alias the raw source; and two Players collide case-insensitively under that exact historical schema-1 and compat-less contract | Fail entry emission with a diagnostic [[self-hosting-15](#self-hosting-15)]. |
| `-o` is absent; the entry path does not alias the raw source; and no exact supported entry contract applies — including absent provenance, exact 0.9.0 or 5.0.0 through 9.0.0 provenance, and a schema or declaration inconsistent with reviewed provenance | Fail entry emission with a diagnostic naming the unsupported or inconsistent entry contract [[self-hosting-15](#self-hosting-15)]. |
