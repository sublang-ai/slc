...
| Prompt unit | A quoted relay fragment as one relay unit, or one blank-separated paragraph of an instruction or plain-blockquote fragment — a relay unit when every line of the paragraph begins with `>`, otherwise an instruction unit — each keeping its Source position. |
...
| A prefixed item is instead held to its prompt units: every unit of a fragment it carries appears contiguously, its instruction units keep Source order, its relay units — each taken at its last occurrence outside an instruction unit — keep Source order, and every relay unit and bare quoted placeholder line follows the last instruction unit. | That item's authored prompt fragments are out of Source order, or that item is listed as prefixed but a relay precedes an instruction. |
| A prefixed item's units are not already in that order in the Source, its entry is well-formed, and its ID names an item. | That item is listed as prefixed but its prompt is in Source order; the `Prefixed prompts` entry is malformed; or the listed ID is not an item. |
...
Where maintained and fixture Sources are paired with their GEARS packages, when the Source-fidelity check runs over each pair, it shall accept every unmutated maintained pair, plain prose without authored fragments, authored raw-token lines, shared lines contained in larger matched fragments, repeated identical fragments with an order-preserving attribution, and a listed prefixed item whose relays — a separate quoted relay or a blank-separated quoted block inside a fenced instruction — now trail its instructions in LF and CRLF forms alike, while naming the same prefixed layout unlisted, a listing that is a no-op, absent, malformed, or still relay-first, drift from an invented item, a dropped fragment, genuinely reversed nonoverlapping fragments including later repeated occurrences, an unquoted relay with the two-layer GEARS example for an additional standalone token, a non-identifier result property, or mixed result-field ownership [[verification-25](#verification-25)].
