...
| A full-link run's concrete reviewed Playbook target selects a different schema from the compiled FSM phase's pin, or a bare run retains a sibling linked factory. | Reconcile the concrete target when present, produced FSM, and linked factory once, and bake the same schema decision and findings into the generated conformance and prompt tests, never the compiler phase's provenance [[verification-21](#verification-21)]. |
