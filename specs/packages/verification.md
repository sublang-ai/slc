...
| Initial configuration or an entry/predecessor arm targets a compound or parallel state | Follow compound initial children and every parallel region when selecting invocation paths, preserve the authored transition and reached context during replay, and recognize active ancestors when observing the declared target; do not count inactive descendants as entered. |
...
| Initial configuration, public event, or predecessor outcome enters nested compound or parallel parents | Accept the active invocation paths and ancestor targets through real XState execution, retaining findings for inactive non-initial descendants. |
