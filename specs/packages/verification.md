...
| Current schema-3 parallel group | Represent each simultaneously active region by its canonical role id, require those region ids to be pairwise distinct, and appear exactly once in the FSM's `concurrentRoleSets` export in source group and region order. |
| Current schema-3 artifact with no parallel group | Export `concurrentRoleSets` as the exact empty array. |
...
When a compilation-correctness check selects the artifact schema used by generation-specific conformance or continuation probes, the check shall apply one fail-closed decision ([DR-024](../decisions/024-playbook-10-schema-3-adoption.md)):

| Available evidence | Required schema decision |
| --- | --- |
| A running full-link compile knows the concrete link target's owning package, or standalone review has the artifact's own matching pin. | Map only exact reviewed link-target provenance according to the supported entry contracts [[self-hosting-15](self-hosting.md#self-hosting-15)]; never substitute the pin of a compiler phase that produced an intermediate artifact. |
| FSM actor structure is unambiguous. | Select schema 3 for a `Roles` binding or controller [[verification-20](#verification-20)], and schema 1 for a historical `Players` binding. |
| The linked callable factory carries exact immutable `{ artifactSchema: 3, runtimeAbi: 1 }` compatibility. | Treat it as schema-3 evidence. |
| Reviewed provenance, unambiguous FSM structure, or exact factory compatibility supplies more than one schema signal. | Require every supplied signal to agree; report an unsupported present provenance, malformed own compatibility, or disagreement instead of applying a lower-priority fallback. |
| No stronger schema signal exists and the linked callable factory has no own `compat`. | Select the immutable historical schema-1 continuation shape. |
| A direct-Captain continuation has neither reviewed provenance, unambiguous actor-generation structure, nor a linked callable factory. | Leave it unclassified and report the missing schema rather than guessing. |

#### verification-22

When a Captain result omits the ordinary Boss-reply key and differs from the closed controller set by exactly one missing or extra domain key, compilation-correctness checks shall report that precise controller-contract near-miss without selecting the valid controller contract or reporting ordinary Boss-reply or `BOSS_INTERRUPT` requirements ([DR-024](../decisions/024-playbook-10-schema-3-adoption.md)).
...
| A full-link run's concrete reviewed Playbook target selects a different schema from the compiled FSM phase's pin. | Bake the concrete target's schema into the generated conformance and prompt tests, never the compiler phase's provenance [[verification-21](#verification-21)]. |
...
| A schema-3 controller decision state [[verification-20](#verification-20)] contains exactly its authored Results and no compiler-owned `needsBossReply`. | Preserve the acting prompt without metadata, parse the source guards in declaration order, and report no finding [[verification-13](#verification-13)]. |
