...
When checking a compiled `playbook` artifact's GEARS↔FSM conformance, the slc command shall recursively traverse nested and parallel state nodes, recover each leaf's binding by evaluating its `invoke.input` under the machine's resolved initial context, require every `gears` item to map to exactly one executable working leaf of its declared actor kind and every such leaf to reference an existing item, and apply all applicable conformance requirements ([DR-009](../decisions/009-slc-playbook-pipeline-compilation.md), [DR-011](../decisions/011-playbook-1-0-captain-contract-adoption.md), [DR-024](../decisions/024-playbook-10-schema-3-adoption.md), [DR-028](../decisions/028-contract-based-adoption-without-recompilation.md)):
...
| Prefixed item | An item a GEARS `## Prefixed prompts` section lists as `- <ITEM-ID>: relays → tail`, the provenance a prompt-prefix pass appends after moving the item's relayed values after its instructions ([DR-052](../decisions/052-prefixed-prompt-acceptance.md)). |
| Prompt unit | A quoted relay fragment as one relay unit; in an instruction or plain-blockquote fragment, each blank-separated paragraph of only `>`-leading lines a relay unit, and the lines between relay units, without their bounding blank lines, one instruction unit keeping its interior blank lines — each keeping its Source position. |

| Rule | Reported violation |
| --- | --- |
| Every authored fragment's lines appear contiguously and in order inside an item's prompt, each authored fragment text as many times as the Source authors it: counted over nonoverlapping occurrences in the unlisted items and in the prefixed items admitting no tiling, plus the fragments of one tiling chosen per remaining prefixed item, the choices together covering the rest, so one occurrence never stands for two authored fragments. | That fragment — the last authored occurrences of a text short of its count — named by its Source line, was dropped or changed. |
...
| A prefixed item is instead held to a tiling of its prompt by whole fragments in Source order: their instruction units in the region before the trailing relay blocks and their relay units among the trailing blocks or, where the raw layout kept a relay beside instruction text, in place; each unit occurrence used once; blank lines and bare quoted placeholder lines free among the trailing blocks; before each unit exactly the one blank line the pass leaves where a moved relay unit stood, exactly the authored blank lines after a unit of the same fragment that kept its place, or the boundary the Source composed before a fragment; and every alternative tiling kept as the fragment texts it carries. | That item's authored prompt fragments are out of Source order; that item is listed as prefixed but a relay precedes an instruction; or that item's prompt admits more tilings than the checker verifies. |
| A prefixed item's listing is not a no-op — some tiling moved a relay past an instruction, or a bare quoted placeholder line trails — its entry is well-formed, its ID names an item, and its section is the only one. | That item is listed as prefixed but its prompt is in Source order; the `Prefixed prompts` entry is malformed; the listed ID is not an item; or a duplicate section is named by its line. |
...
Where maintained and fixture Sources are paired with their GEARS packages, when the Source-fidelity check runs over each pair, it shall accept every unmutated maintained pair, plain prose without authored fragments, authored raw-token lines, shared lines contained in larger matched fragments, repeated identical fragments with an order-preserving attribution, a listed prefixed item whose relays — a separate quoted relay or a blank-separated quoted block inside a fenced instruction — now trail its instructions in LF and CRLF forms alike, and listed items whose prompt another item's fragment also tiles, that repeat a paragraph or a relay block, that mirror each other, that carry one fragment text across seven items or across sixty-five with one listed, that keep bare relays authored through prose alone or adjacent, that join fragments with the boundary the Source authored including none, that keep a relay in place with its authored blank lines, that keep an instruction's interior blank lines, or that keep a Source boundary of more than one blank line, while naming the same prefixed layout unlisted, a listing that is a no-op, absent, malformed, duplicated, or still relay-first, drift from an invented item, a dropped fragment, a repeated relay block, a mirrored item's counterpart, one fragment of a two-fragment prompt, or one of seven identical items deleted, interior or kept-relay blank lines shortened or lengthened, genuinely reversed nonoverlapping fragments including later repeated occurrences, an unquoted relay with the two-layer GEARS example for an additional standalone token, a non-identifier result property, or mixed result-field ownership [[verification-25](#verification-25)].
