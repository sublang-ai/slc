...
| --- | --- |
| A running full-link compile knows the concrete link target's owning package, or standalone review has the artifact's own matching pin whose currency validator reports current [[pinning-2](pinning.md#pinning-2)]. | In this verification-only schema decision, map exact `@sublang/playbook@0.10.0`, `@sublang/playbook@1.0.0`, `@sublang/playbook@2.0.0`, `@sublang/playbook@3.1.0`, and `@sublang/playbook@4.0.0` provenance to schema 1 and exact `@sublang/playbook@10.0.0` provenance to schema 3, report every other present provenance as unsupported, and never substitute the pin of a compiler phase that produced an intermediate artifact. |
