...
Where a dormant role-bearing schema-3 registry fixture and deterministic live host capabilities are available, when automatic release-readiness checks probe the entry contract without a Boss turn, the gate shall exercise one quiescent consumer lifecycle before production emission is enabled ([DR-024](../decisions/024-playbook-10-schema-3-adoption.md), [DR-025](../decisions/025-defer-playbook-10-activation.md)):
...
| Run portion is selected | Select the updated entry when Compile and Update ran, the freshly compiled entry when Compile ran but Update was skipped, or the committed reference when Compile was skipped; copy that schema-1 entry, its bundle, and the buggy sample into a scratch run directory; require at least one entry-declared player; bind every declared player through a repeated `--player <id>=<agent>` and the Captain through `--captain <agent>`; and drive the entry through the installed Playbook 4 host with real agents using `playbook run ./<basename>.ts "<task>" --player <id>=<agent> ... --captain <agent> --json` [[self-hosting-14](self-hosting.md#self-hosting-14)]; require a terminal result, require the scripted workflow to initialize the run directory as a Git repository without an agent call and land at least one commit, and require the repaired sample to pass the compiled median driver ([DR-020](../decisions/020-playbook-4-0-adoption.md), [DR-025](../decisions/025-defer-playbook-10-activation.md)). |
...
| --- | --- |
| Automatic release checks | Inside the installed consumer, import the committed English schema-1 reference entry through its artifact-local dependencies [[release-11](#release-11)], construct its runtime with a scratch `captainOptions.cwd`, initialize it with fake host ports, drive one Boss turn to a terminal outcome, dispose it, require at least one player call, and require the scripted state to initialize a repository in that run directory without an agent call as part of the publication gate [[release-12](#release-12)]. |
| Opt-in local gate | Compile a minimal workflow through the installed executable and require a loadable entry; require an unchanged repeat to report exactly `up to date`, preserve the active marker, and leave every recorded phase target equal to its snapshot; unless Update is skipped, require a repeat after a manual GEARS refinement to report both Update and Reuse, preserve and record the refinement in a new build, and leave the updated entry loadable; run the selected schema-1 entry through the installed Playbook 4 host with positional module and task arguments plus explicit `--player` and `--captain` bindings to a terminal outcome whose repaired sample passes a compiled median check; and use the committed reference set when Compile is skipped [[release-17](#release-17)], thereby exercising the compiler-change gate required by [[release-13](#release-13)]. |

### Dormant schema-3 readiness

#### release-22

Where the source suite places the dormant role-bearing schema-3 registry fixture and its linked factory in a scratch Git repository with one clean baseline, when dormant release-readiness acceptance runs, the acceptance shall require the exact roles, concurrency, option validation, and one exact live-capability construction, initialize and dispose one causal-root runtime without a Boss turn, observe no call-port, repository-operation, or ledger-write use, require detached empty-ledger snapshots, and leave repository history and status unchanged [[release-21](#release-21)].
