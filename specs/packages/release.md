...
| --- | --- |
| Stage and lineup selection | Refuse an invocation that selects no stage; bind every player and the Captain explicitly so the maintainer's host defaults cannot change the test; and require exactly the agent CLIs that the bound lineup invokes. |
