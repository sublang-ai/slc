...
| Run portion is selected | Select the updated entry when Compile and Update ran, the freshly compiled entry when Compile ran but Update was skipped, or the committed reference when Compile was skipped; copy that schema-3 entry, its bundle, and the buggy sample into a scratch run directory; require at least one entry-declared role; enable the entry in the installed Playbook host's configuration with every declared role and the Captain bound explicitly to real agents, so the maintainer's personal run defaults cannot change the test; drive the entry to a terminal result by invoking its effective slash command with the task [[self-hosting-14](self-hosting.md#self-hosting-14)]; require the scripted workflow to initialize the run directory as a Git repository without an agent call and land at least one commit; and require the repaired sample to pass the compiled median driver ([DR-024](../decisions/024-playbook-10-schema-3-adoption.md), [DR-027](../decisions/027-complete-playbook-10-activation.md)). |
