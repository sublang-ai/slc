...
### PHEXEC-20
Verifies: [PHEXEC-1](../dev/phase-execution.md#phexec-1), [PHEXEC-2](../dev/phase-execution.md#phexec-2), [PHEXEC-3](../dev/phase-execution.md#phexec-3), [PHEXEC-6](../dev/phase-execution.md#phexec-6), [PHEXEC-11](../dev/phase-execution.md#phexec-11), [PHEXEC-14](../dev/phase-execution.md#phexec-14), [PHEXEC-15](../dev/phase-execution.md#phexec-15)

When the slc command interprets compile and link fixture phases, the agent prompt shall embed the phase or link definition verbatim as authoritative, establish every clause of the [PHEXEC-14](../dev/phase-execution.md#phexec-14) agent contract together with permission to run definition-called tools and read cited content, end with exactly one canonical `sublang.slc.workspace.v1` suffix containing respectively the exact compile or link role set with contiguous indexed roles, cross-bound absolute logical and physical reads and identities, and one distinct fresh physical write sink, and add no phase-specific rules of slc's own; an interpreted fixture that writes a differing canonical logical target instead of or in addition to that physical sink shall fail as a write-scope violation.
...
### PHEXEC-35
Verifies: [PHEXEC-3](../dev/phase-execution.md#phexec-3), [PHEXEC-6](../dev/phase-execution.md#phexec-6), [PHEXEC-25](../dev/phase-execution.md#phexec-25), [PHEXEC-34](../dev/phase-execution.md#phexec-34)

This scenario exercises a seeded compile or link turn only. [PHEXEC-29](../dev/phase-execution.md#phexec-29)'s update-capable compile — the Boss request carrying `sublang.slc.update-request.v1` — has no compiled transport yet and so is verified by nothing; it is deferred with the rest of the scoped-update transport work ([IR-021](../iterations/021-incremental-compilation.md#deferred)).

Where pinned `legacy`, `session-v1`, and `composed-v2` meta-phase artifacts are driven through the compiled executor over a fake agent transport that captures transported prompts, when a seeded compile or link turn reaches a transformation-performing Player or direct Captain call, the transported prompt shall carry the artifact's composed GEARS-derived body plus exactly one final canonical `sublang.slc.workspace.v1` suffix whose read roles are exactly the applicable compile or link set with contiguous indexes, whose logical and physical paths and identities cross-bind the request, and whose one distinct fresh write names the target or linked logical path and sole physical file sink, and an agent that writes exactly that sink shall map the run to `ok`; the Boss request shall retain only canonical logical paths, while a routing-only Captain call carrying an explicitly empty `allowedTools` and every hidden judge call shall receive its composed prompt unchanged; a performing transport that writes a differing canonical logical target instead of or in addition to the physical sink shall fail as a write-scope violation.
...
Where interpreted and compiled fixture phases declare scoped-update support and emit one schema-exact `sublang.slc.update.v1` payload respectively through the exact final `sublang.slc.interpreted-result.v1` suffix on existing Cligent final text and one reserved Playbook `emitTelemetry` event, when their executors complete, the slc command shall remove the valid interpreted suffix before status parsing, return the same generic metadata beside ordinary status and diagnostics, divert every compiled reserved event through the protected SLC-update sink, and put the payload or envelope in no Playbook turn result, `playbook.trace`, status, or ordinary diagnostics; a duplicate, misplaced, unterminated, unknown-field, or malformed interpreted envelope and a malformed or duplicate compiled reserved event shall preserve ordinary status with no metadata and one host diagnostic, while an immutable compiled runtime that emits no such topic shall keep its exact terminal-result mapping and return no metadata.
