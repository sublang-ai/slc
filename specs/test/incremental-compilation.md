...
While [INCR-39](../dev/incremental-compilation.md#incr-39) withholds scoped update, this scenario is inapplicable and [INCR-38](#incr-38) holds in its place.
...
### INCR-38
Verifies: [INCR-3](../user/incremental-compilation.md#incr-3), [INCR-39](../dev/incremental-compilation.md#incr-39)

Where a fixture pipeline declares a valid `## Update` contract and its executor emits a bound trace on an ordinary run, when the user edits one input unit and repeats the invocation, the recorded step shall carry that ordinary trace, no execution request shall carry an update object, and the repeated step shall record `origin: "ordinary"`; and where a request carrying an update object reaches any exported consumer — the phase execution boundary, the interpreted executor, the compiled executor, the prompt builder, or workspace binding or validation — each shall independently fail with a diagnostic naming the withholding, invoking no agent, writing no target, and certifying no binding.
