...
### INCR-38
Verifies: [INCR-39](../dev/incremental-compilation.md#incr-39)

Where a fixture pipeline declares a valid `## Update` contract and its executor emits a bound trace on an ordinary run, when the user edits one input unit and repeats the invocation, the recorded step shall carry that ordinary trace, no execution request shall carry an update object, the repeated step shall record `origin: "ordinary"`, and a phase execution request carrying an update object shall fail with a diagnostic naming the withholding rather than invoking the executor.
