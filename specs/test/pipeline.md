...
### PIPE-25
Verifies: [PIPE-12](../dev/pipeline.md#pipe-12), [PIPE-18](../dev/pipeline.md#pipe-18), [COMPILE-2](../user/compiler.md#compile-2)
...
Where no lineage metadata exists, when the slc command is run with `--normalize`, the slc command shall execute the built-in normalization definition first — receiving the raw source and the entry-phase definition as a read-only reference — write the normalized source into the artifact directory under the entry phase's source name, and run the entry phase from that file; whereas where the invocation's source is that same normalization target — named directly or reached through a symbolic link — the slc command shall refuse the run before invoking any executor; whereas where a link rebound after planning but before promotion is authorized makes them the same file, the slc command shall reject the candidate rather than promote it, since the collision cannot be observed earlier; whereas where the rebind happens after authorization — at or after the published manifest, including before the first rename — the slc command shall complete promotion forward, leave the original raw input's own bytes unmodified, publish the accepted lineage, and leave no staged residue; in both cases it shall leave the raw input byte-for-byte and leave no staged residue.
