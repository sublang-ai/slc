...
Where a clean registry install resolves exact `@sublang/playbook@3.1.0` and the repository vendors the adopted shared definitions, when the adoption acceptance runs, the reserved `slc` and `playbook` references shall both resolve to the vendored `text2gears`, `gears2fsm`, and `link` set corresponding to that installed release with explicit pin inputs retained, all three reviewed artifact bundles shall pass all generated verification, every corresponding pin shall be current with exact 3.1.0 link-target provenance and the recorded shared-engine runtime dependency, and changing any dependency, definition, bundle, or pin component back to its 2.0.0 form shall make the set fail acceptance rather than pass as a mixed version.
