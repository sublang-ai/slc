...
meta-pipeline and its `phase` linked format in the `self-hosting` packages,
...
Where a fixture reserves an `slc` pipeline that chains `text2gears` and `gears2fsm` and a `link.md` emitting `phase`, when the user runs `slc slc <definition>` and then the same run with an explicit `--link <target>`, the slc command shall write the `fsm` object and a `phase` artifact that resolves to the phase-runner facade at their [DR-001](../decisions/001-slc-pipeline-layout-naming-invocation.md#output-locations) locations, and shall still fail the run when the `slc` reference itself does not resolve.
