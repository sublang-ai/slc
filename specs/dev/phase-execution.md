...
When a run is planned, the slc command shall resolve every filesystem operand against the invocation working directory once and use those canonical paths for identity, protection, prompts, and history; shall refuse a plan whose writes — the chain targets, exactly the files this invocation's host will write (the runnable entry module and the verification files when they will actually be emitted and nothing when they will not), and the directories it will create — name the same prospective physical file pairwise or alias an input no earlier step produces, deciding every such conflict, including whether entry emission is planned, with one relation that resolves absent paths through their nearest existing ancestors, where the protected inputs include the pipeline's pin index, every local path any pin record names whether or not its phase is scheduled — package locators included, shielded from planned outputs while their runtime integrity remains the per-phase pin re-evaluation's to enforce — and the installed verifier-support sources a planned emission reads; shall refuse any planned write that names or enters a `.slc` or `.slc-verify` namespace anywhere on the filesystem — its own bundle's, a foreign bundle's, or a direct link's destination, judged on the canonical and prospective forms alike, because a write through any of them could resurrect a superseded currentness marker or corrupt a verifier closure, and the same ban holds at the public phase boundary: a bare `runPhase` refuses such a target before its executor — and any protected input inside the invocation's own reserved directories, deciding containment lexically on the canonical paths first and then on the prospective physical form resolved through the nearest existing ancestor; shall refuse a planned write with components still missing below a nearest existing anchor that is not a directory, or below an existing symbolic link that does not resolve — genuine absence, never an unresolved link, is what a prospective location may assume — a planned directory whose place a file holds, and a planned file that would contain another planned write; and shall create no directory — the artifact directory included — for a plan that fails validation; and shall refuse to invoke an executor while the target is a symbolic link, another non-regular file, or a hard-linked file, is physically the same file as any protected path — the request's inputs, the chain definitions, the declared semantic inputs, the plan's immutable inputs, and every earlier product — resolves prospectively to the location of an absent protected path, or lies inside a protected directory, failing with a reason naming the conflict; a protected path's change during execution shall be detected structurally — inode, link count, and content digest — so a swap to byte-identical content still fails the phase, and a protected path that cannot be fully observed — or a target whose prospective form cannot be resolved — fails the phase before its executor runs; reuse and the produced-target postcondition likewise accept only a single-link regular file, and every host-owned deterministic write goes through one writer that refuses a symbolic-link or multi-link leaf before truncating and reaches the host-managed `.slc` and `.slc-verify` directories only as real directories, never through a symbolic link; the verifier-support closure a planned emission writes is declared as managed writes and preflighted with the plan — a blocked `.slc-verify` root or unsafe support leaf refuses the run before any executor — while the store's own `.slc` writes remain the managed carve-out those recording primitives enforce ([DR-003](../decisions/003-slc-phase-execution.md), [DR-021](../decisions/021-incremental-compilation.md)).
