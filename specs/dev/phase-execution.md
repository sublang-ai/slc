...
When the slc command seeds a compiled phase's non-interactive turn ([PHEXEC-23](#phexec-23)), the slc command shall pass one Boss turn whose text states the request kind — compile or link — in prose and carries the full request as a single JSON line introduced by `Request: `, with the request's canonical logical workspace locators resolved to absolute host paths, so any compiled `playbook` artifact's classifier — or a deterministic consumer — recovers the exact semantic phase input without host-specific parsing while any distinct staged physical binding remains host-owned ([DR-005](../decisions/005-slc-self-hosting-meta-pipeline.md#linked-phase-artifact-contract), [DR-021](../decisions/021-incremental-build-records-scoped-updates.md#build-lineage)).
