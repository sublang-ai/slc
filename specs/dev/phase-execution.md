...
When interpreting a phase, the slc command shall prompt a coding agent, reached through Cligent [[1]], with the phase or link definition, the canonical semantic request, and exactly one final `SLC_WORKSPACE_BEGIN`/canonical-one-line-`sublang.slc.workspace.v1`/`SLC_WORKSPACE_END` physical binding, and the agent shall perform the transformation and write its one host-bound physical sink ([DR-004](../decisions/004-slc-interpreted-phase-execution.md#interpreter), [DR-021](../decisions/021-incremental-build-records-scoped-updates.md#physical-workspace-binding)).
...
- canonical logical paths are semantic identifiers without filesystem authority, while the exact `sublang.slc.workspace.v1` binding is the sole filesystem authority for named host-supplied paths and the write sink;
...
When interpreting a phase, the slc command shall permit the agent to invoke the deterministic tools or commands the definition calls for and to read the content the definition cites or references, as part of following the definition, and shall treat that readable closure as the phase's semantic input closure; any read-only content not enumerated in the physical binding remains definition-authorized for ordinary execution but makes the closure open and ineligible for reuse or scoped update ([DR-004](../decisions/004-slc-interpreted-phase-execution.md#interpreter), [DR-021](../decisions/021-incremental-build-records-scoped-updates.md#physical-workspace-binding)).
...
Where the slc command configures compiled execution from a current pin, when it selects the runtime contract profile, the slc command shall select `legacy` only for absent link-target provenance or exact `@sublang/playbook@0.9.0` provenance, select the final six-port `composed-v2` profile only for exact `@sublang/playbook@0.10.0`, `@sublang/playbook@1.0.0`, `@sublang/playbook@2.0.0`, `@sublang/playbook@3.1.0`, or `@sublang/playbook@4.0.0` provenance — the releases that carry the final six-port contract — reject every other provenance, including `@sublang/playbook@1.3.0` and `@sublang/playbook@3.0.0`, until mapped by a later decision, and neither infer the profile from callable runtime members nor retry a failed initialization under another profile ([DR-010](../decisions/010-playbook-runtime-contract-evolution.md#runtime-profiles-and-root-phase-sessions), [DR-011](../decisions/011-playbook-1-0-captain-contract-adoption.md#immutable-profile-boundary), [DR-017](../decisions/017-playbook-2-0-thin-runtime-adoption.md#immutable-profile-boundary), [DR-018](../decisions/018-playbook-3-1-adoption.md), [DR-020](../decisions/020-playbook-4-0-adoption.md#provenance-mapping)).

### PHEXEC-29

When the slc command seeds a compiled phase's non-interactive turn ([PHEXEC-23](#phexec-23)), the slc command shall pass one Boss turn whose text states the request kind — compile or link — in prose and carries the full request as a single JSON line introduced by `Request: `, with canonical logical workspace locators resolved to absolute host paths and, only for an update-capable compile, the exact `sublang.slc.update-request.v1` object, so any compiled `playbook` artifact's classifier — or a deterministic consumer — recovers the exact semantic phase input without host-specific parsing while `sublang.slc.workspace.v1` and every physical path remain host-owned and absent from the Boss request ([DR-005](../decisions/005-slc-self-hosting-meta-pipeline.md#linked-phase-artifact-contract), [DR-021](../decisions/021-incremental-build-records-scoped-updates.md#update-request), [DR-021](../decisions/021-incremental-build-records-scoped-updates.md#physical-workspace-binding)).
...
Where a compiled phase makes a Player call or a `composed-v2` transformation-performing direct Captain call — the latter carrying no source-owned `allowedTools` property ([PHEXEC-31](#phexec-31)) — the slc command shall append exactly one final `SLC_WORKSPACE_BEGIN`/canonical-one-line-`sublang.slc.workspace.v1`/`SLC_WORKSPACE_END` contract carrying the ordered, uniquely roled, absolute logical and physical reads with their identities and the one target or linked write binding defined by [DR-021](../decisions/021-incremental-build-records-scoped-updates.md#physical-workspace-binding), plus the interpreted contract's write-scope rules ([PHEXEC-14](#phexec-14)); under execution staged by [INCR-8](incremental-compilation.md#incr-8) the physical binding shall name staged candidate predecessors and sink while unchanged read-only inputs retain canonical physical paths and the Boss request retains [PHEXEC-29](#phexec-29)'s canonical logical locators, writing a differing canonical logical path shall be a [PHEXEC-6](#phexec-6) scope violation, and otherwise the physical and logical paths shall coincide; because the linked artifact composes only host-agnostic prompts while the host alone owns workspace specifics, the appended contract shall carry no guard names, result-property schema, or adjudicator instructions, shall enter no artifact, trace identity, result, status, or diagnostics, and a routing-only Captain call carrying explicitly empty `allowedTools` and every hidden judge call shall cross with their composed prompts unchanged ([DR-005](../decisions/005-slc-self-hosting-meta-pipeline.md#linked-phase-artifact-contract), [DR-012](../decisions/012-playbook-routing-control-separation.md#routing-authority-and-presentation-ownership), [DR-021](../decisions/021-incremental-build-records-scoped-updates.md#build-lineage)).
...
Where a phase definition declares a scoped-update contract, when the shared host executor returns its phase result, the executor shall preserve status and diagnostics and optionally add one `metadata` object whose sole field is the schema-exact `"sublang.slc.update.v1"` trace; interpreted execution shall extract it only from one exact final `SLC_RESULT_BEGIN`/canonical-one-line-`sublang.slc.interpreted-result.v1`/`SLC_RESULT_END` suffix on the existing Cligent final text, remove a valid suffix before existing status and diagnostic parsing, treat a malformed suffix as absent metadata plus a host diagnostic, and make no Cligent API change; the compiled host adapter shall divert every `emitTelemetry` event with that topic into a dedicated protected SLC-update metadata sink after mapping the unchanged exact Playbook turn result, accept exactly one schema-valid reserved event, and treat a malformed payload or duplicate reserved event as absent metadata plus one host diagnostic; both paths shall exclude every payload and reserved envelope from `playbook.trace`, status, and ordinary diagnostics, while an unsupported runtime or absent value shall preserve ordinary success with no update trace ([DR-021](../decisions/021-incremental-build-records-scoped-updates.md#executor-metadata-transport), [DR-005](../decisions/005-slc-self-hosting-meta-pipeline.md#linked-phase-artifact-contract), [DR-010](../decisions/010-playbook-runtime-contract-evolution.md#port-policy-and-diagnostic-privacy)).
