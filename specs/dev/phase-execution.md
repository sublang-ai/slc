...
Where the slc command configures compiled execution from a current pin, when it selects the runtime contract profile, the slc command shall select `legacy` only for absent link-target provenance or exact `@sublang/playbook@0.9.0` provenance, select the final six-port `composed-v2` profile only for exact `@sublang/playbook@0.10.0`, `@sublang/playbook@1.0.0`, `@sublang/playbook@2.0.0`, or `@sublang/playbook@3.1.0` provenance — the releases that carry the final six-port contract — reject every other provenance, including `@sublang/playbook@1.3.0` and `@sublang/playbook@3.0.0`, until mapped by a later decision, and neither infer the profile from callable runtime members nor retry a failed initialization under another profile ([DR-010](../decisions/010-playbook-runtime-contract-evolution.md#runtime-profiles-and-root-phase-sessions), [DR-011](../decisions/011-playbook-1-0-captain-contract-adoption.md#immutable-profile-boundary), [DR-017](../decisions/017-playbook-2-0-thin-runtime-adoption.md#immutable-profile-boundary), [DR-018](../decisions/018-playbook-3-1-adoption.md)).
