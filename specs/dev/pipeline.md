...
When a full or full-link invocation carries `--normalize` or its entry source is a raw input ([PIPE-6](#pipe-6)), the slc command shall schedule one generic normalization step ahead of the entry phase, driven by the pipeline-agnostic definition shipped with slc, executing it to write, exactly reusing, or as part of the complete lineage explicitly adopting `<art-dir>/<basename>.<entry-source-format><entry-source-ext>` as the entry phase's source and supplying the entry-phase definition as a protected read-only reference input; whereas where that target and the invocation's own source are the same file — by path or through a symbolic link — the slc command shall refuse the run with a diagnostic naming the collision rather than schedule a step that consumes the user's raw input, and shall re-observe that non-collision with the rest of the candidate basis until promotion is authorized, after which promotion completes forward because a later rebind leaves the original input unmodified ([COMPILE-7](../user/compiler.md#compile-7), [DR-013](../decisions/013-normalize-and-pass-phases.md), [DR-014](../decisions/014-cwd-output-invocation-defaults-entry-emission.md), [DR-021](../decisions/021-incremental-build-records-scoped-updates.md), [PHEXEC-33](phase-execution.md#phexec-33)).
