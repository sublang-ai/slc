...
Where the invocation is a non-lineage form and the produced `fsm` module cannot be imported for derivation, when deterministic emission runs, the slc command shall report a diagnostic and emit no introspection test while leaving the run outcome unchanged ([DR-009](../decisions/009-slc-playbook-pipeline-compilation.md)).
Where the invocation is eligible to establish canonical build lineage and its complete planned deterministic inventory cannot be emitted or pass its applicable checks, when the slc command evaluates the candidate, the slc command shall discard the complete staged candidate without promoting an artifact, deterministic derivative, source snapshot, or build record ([DR-021](../decisions/021-incremental-build-records-scoped-updates.md#build-lineage)).
...
Where the host supplies explicit physical semantic-input and candidate verification-product paths plus their eventual logical artifact layout, when the deterministic verification facility generates and checks those products, the slc command shall write and return only a stable ordered inventory of the supplied candidate destinations, render the checker-support and artifact references from the logical layout so no candidate locator enters emitted bytes, produce the same support and tests required by [VERIFY-2](#verify-2), [VERIFY-4](#verify-4), [VERIFY-5](#verify-5), [VERIFY-6](#verify-6), and [VERIFY-12](#verify-12) for the canonical logical layout, expose before semantic execution stable `generator:verification`, `checker:load-integrity`, and `checker:verification` identity descriptors whose exact file or tree bytes or canonical values cover every output- or acceptance-affecting generation, support, load-check, suite-runner, and configuration dependency, provide a load-integrity runner that reports every unresolvable relative import from the supplied candidate modules and an emitted-suite runner that copies the exact staged and retained inventory into a caller-authorized private mirror of the logical layout, runs exactly the supplied generated-test inventory from that view, removes the view on every outcome, and reports its outcome and diagnostics, and in candidate mode neither write a canonical product nor update, accept, or promote build lineage ([DR-021](../decisions/021-incremental-build-records-scoped-updates.md#sublangslcbuildv1)).
...
When parsing GEARS items, the slc command shall recognize the optimizer-introduced script kind — the literal `Captain shall run:` clause with a blockquoted static script and exactly two exit-status `Results:` guards — and when checking GEARS↔FSM conformance it shall pair each script item with exactly one `script` actor state whose `command` carries the blockquote verbatim and whose result map preserves the item's two guards in declared order with no `needsBossReply`, reporting drift, an agent-actor realization of a script item, and a script state realizing a non-script item as findings ([DR-013](../decisions/013-normalize-and-pass-phases.md)).
