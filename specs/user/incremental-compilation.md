...
Where an artifact directory has malformed, unsupported, orphaned, wrong-typed, or symbolic-link lineage metadata, is recorded for another source locator, has an inconsistent source snapshot, contains a managed artifact changed outside an accepted incremental run, or belongs to an adopted lineage whose current plan topology is incompatible or whose source, build inputs, or build identity drifted after adoption, when the user runs the canonical full or full-link pipeline without `--adopt`, the slc command shall refuse to overwrite the bundle if `--rebuild` is absent and identify the conflict with both `--adopt` and `--rebuild` guidance only for an unchanged-source lineage satisfying every other adoption precondition, otherwise with `--rebuild` guidance alone, and shall run the complete pipeline without reuse or scoped update and replace the binding only after success if `--rebuild` is present ([DR-021](../decisions/021-incremental-build-records-scoped-updates.md#exact-reuse-and-conflicts), [DR-021](../decisions/021-incremental-build-records-scoped-updates.md#explicit-adoption)).
