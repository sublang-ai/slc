...
- [ ] Land dormant `composed-v3` phase-host, result, capability, CLI, and integration-fixture readiness while exact 10.0.0 compiled execution remains fail-closed.
...
2. [ ] Add dormant `composed-v3` phase construction, fail-closed capability seams, structured-result handling, CLI wiring, and isolated integration fixtures without admitting exact 10.0.0 provenance.
...
4. [ ] Assemble the complete atomic reviewed set, run its deterministic and real-agent acceptance, record the evidence below, mark this record `Done`, check every remaining deliverable and task, and create the single cutover commit.
