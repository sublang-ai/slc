...
The runtime reads any own `meta.playbook` namespace as public state metadata, so an empty or null root namespace also fails normalization.

## Decision

- Make that boundary explicit in conformance: the machine root omits an own `meta.playbook` namespace regardless of its value; public playbook metadata belongs to state nodes declared under `states`.
- Preserve the root's XState `id`, description, and other metadata namespaces, and existing flat, structured, and historical state-node identity rules.
