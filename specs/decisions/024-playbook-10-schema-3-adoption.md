...
Current compilation shall accept `Roles`, reject removed player-alias syntax and canonical-id collisions, require every canonical lowercase local role id to match `[a-z][a-z0-9_-]*` and differ from the host-reserved `captain`, preserve those ids through `meta.playbook.role` and `invoke.input.role`, and carry declared concurrent-role sets and the controller decision-state exception into deterministic verification [[4]][[5]][[11]][[12]].
Deterministic verification shall recognize that controller exception through Playbook 10's shared structural discriminator: a Captain decision state whose domain result keys, after excluding compiler-owned `needsBossReply`, are exactly `respond`, `resume`, `start`, `switch`, `dismiss`, `deliver`, and `runtime`; GEARS prose and a proper subset or superset of those keys shall not independently select the controller contract [[5]].
