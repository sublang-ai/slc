...
The schema-3 host capabilities contain authority, repository, and effect-ledger seams, and the runtime adds the `unresolved-effect` result, terminal state descriptions, role bindings, retained-generation metadata, adoption and control operations, and unresolved-effect inspection [[2]][[3]][[7]].
...
The `composed-v3` profile shall construct a Captain-hosted schema-3 linked runtime with a plain configured-options object and a live current-host `hostCapabilities` object containing exactly authority, repository, and effect-ledger capabilities, initialize the same six-port causal root session with canonical local-role semantics, and never serialize the live capabilities into options, machine input, or snapshots [[2]][[3]].
...
SLC's compiled `composed-v3` phase host shall support only the three rebuilt roleless shared-factory meta-phase artifacts with empty configured options and fail-closed repository and effect-mutation seams, and shall reject a bespoke implementation or an artifact that lacks the required shared-factory compatibility record or requires a delegated role, concurrent cohort, runtime option, repository operation, or effect-ledger write; emitted role-bearing or bespoke playbooks remain runnable through a Playbook schema-3 host rather than through SLC's non-interactive phase host.
Because schema-3 authority requires a canonical worktree and Git directory, compiled `composed-v3` phase execution shall require the invocation working directory to belong to a resolvable Git worktree and shall fail before construction otherwise; interpreted execution and older profiles retain their existing working-directory behavior.
...
For schema-3 output only, it also supersedes [DR-017](017-playbook-2-0-thin-runtime-adoption.md)'s verbatim source-name `requiredRoleIds` and lowercase-to-verbatim entry-binding shim.
...
- A pinned `composed-v3` phase can run only from a resolvable Git worktree; a repository-free working directory must use an unpinned interpreted phase or an older complete dependency closure.
