...
Artifact verification shall select a schema from the artifact's own reviewed full-link target provenance, unambiguous actor-generation structure, and exact shared-factory compatibility rather than from a compiler phase's pin; mutually inconsistent or unsupported present signals shall fail closed, a compat-less linked factory with no schema-3 signal shall retain the historical schema-1 continuation, and a direct-Captain artifact with neither a linked factory nor another generation signal shall remain unclassified.
...
A schema-3 registry may additionally carry Playbook's optional host-owned `summaryPolicy` with the exact `stateCountLabels`, `copyPasteGuardNames`, and `savedCountsLine` member shape; SLC verification may leave its formatter prose semantically un-compared, but shall not accept a malformed policy as host-supported [[12]].
