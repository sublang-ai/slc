...
SLC's compiled `composed-v3` phase host shall support only the three rebuilt roleless shared-factory meta-phase artifacts with empty configured options, the engine's canonical empty effect ledger, and fail-closed `runExclusive`, `runDeferred`, and ledger-write seams, and shall reject a bespoke implementation or an artifact that lacks the required shared-factory compatibility record or requires a delegated role, runtime option, repository operation, or effect-ledger write; emitted role-bearing or bespoke playbooks remain runnable through a Playbook schema-3 host rather than through SLC's non-interactive phase host.
