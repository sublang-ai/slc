...
Keeping outputs under the invocation directory avoids that overwrite: outputs follow the invocation, not the source.
...
Playbook 1.0's `run` CLI consumes any module whose default export matches its registry-entry shape (`id`, `command`, `intent`, `requiredRoleIds`, `validateOptions`, `createRuntime`) [[1]], defaults every player and the captain per its PBCLI-19 [[3]], and accepts the task as a positional [[2]] — so a deterministically derivable entry module is the only missing piece between a compiled bundle and a running workflow.
...
- `-o` overrides and every in-directory naming rule ([DR-001](001-slc-pipeline-layout-naming-invocation.md) artifact names, [DR-013](013-normalize-and-pass-phases.md) `.raw`/`.opt` names) are unchanged.

### 2. Raw entry sources

Extends the generic-input-normalization decision in [DR-013](013-normalize-and-pass-phases.md); amends [DR-001](001-slc-pipeline-layout-naming-invocation.md)'s source-name refusal for the entry role only.
...
Amends the pass-scheduling decision in [DR-013](013-normalize-and-pass-phases.md) ("passes run only on request").
...
- Every other pipeline keeps [DR-002](002-slc-link-phases.md): without `--link`, the run stops at the compile-chain output.
...
- After a successful full-link of the reserved `playbook` pipeline with the linked artifact at its canonical path, `slc` deterministically emits `<cwd>/<basename>.ts` — a runnable entry module whose default export satisfies Playbook's registry-entry contract [[1]].
- Field derivation:

| Field | Derivation |
| --- | --- |
| `id`, `command` | `<basename>` — stable across recompiles, as parked-session resume requires [[4]] |
...
- `playbook run ./workflow.zh.ts "<task>"` performs it; players and the captain default per Playbook's PBCLI-19 [[3]], so only the roles a user wants to change need `--player`.
...
- Tests and tooling that pinned [DR-001](001-slc-pipeline-layout-naming-invocation.md)'s beside-source placement re-anchor to the CWD rule.

## References

[1]: https://github.com/sublang-ai/playbook/blob/v1.0.0/specs/decisions/009-generic-playbook-cli-and-registry.md#1-registry-entries-become-the-playbook-manifest "Playbook registry-entry contract (v1.0.0)"
[2]: https://github.com/sublang-ai/playbook/blob/v1.0.0/specs/user/playbook-cli.md#pbcli-18 "Playbook non-interactive run contract (v1.0.0)"
[3]: https://github.com/sublang-ai/playbook/blob/v1.0.0/specs/user/playbook-cli.md#pbcli-19 "Playbook run agent defaults (v1.0.0)"
[4]: https://github.com/sublang-ai/playbook/blob/v1.0.0/specs/decisions/014-durable-one-shot-run-sessions.md "Playbook durable one-shot run sessions (v1.0.0)"
