...
- `source.hash`, `source.snapshotHash`, and the exact bytes of the source and `.slc-source` are equal when promotion is authorized. Promotion is forward-only past that point, so a source path rebound during the rename sequence is outside this equality; the original input is not modified by such a rebind, and detecting it would require a durable non-alias fact the manifest deliberately does not carry.
