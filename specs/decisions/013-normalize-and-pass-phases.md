...
- A phase whose `## Formats` source and target formats are equal is a **pass phase**.
  Its filename is its non-empty portable pass name (`<name>.md`, e.g. `optimize.md`), which may otherwise be named freely but may not contain a path separator or use `.` or `..`; the `<source>2<target>.md` rule applies only to format-changing phases.
