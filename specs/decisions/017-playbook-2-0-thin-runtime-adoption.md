...
Playbook 2.0.0 keeps the six-port causal-session init boundary, the structured turn result, and `resumePlaybookCall` unchanged, so it does not alter any surface the `composed-v2` profile gates [[4]].
...
Third, the release raises the registry dependency closure to `@sublang/cligent` `^0.16.0`, whose contract keeps the explicit player resume and isolated tool-restricted Captain calls SLC's ports already implement [[1]].
...
Finally, the demo exposed a role-identity gap between two published contracts: `text2gears` capitalizes English player names [[5]], so [[self-hosting-15](../packages/self-hosting.md#self-hosting-15)] emits `requiredRoleIds` like `Coder` verbatim, while `slc/link.md`'s default player binding resolves acting players to their lowercased names [[6]] and Playbook's `playbook run` host keys its agents by the verbatim ids with exact-case `callPlayer` lookup [[7]] — so a cased English source fails at its first delegated state (`unknown player coder`) even though caseless sources run.
...
`@sublang/playbook@1.3.0` shall not be mapped: SLC never installed it, no reviewed artifact records it, and [DR-010](010-playbook-runtime-contract-evolution.md)'s fail-closed rule governs it like every other unmapped provenance, without shape inference or initialization retry.
[[phase-execution-30](../packages/phase-execution.md#phase-execution-30)] shall be corrected to state this complete mapped set, closing its current 0.10.0-only drift from the configured selector.
...
The host's thrown-turn error path remains reserved for what PBRT-47 [[2]] keeps as control-plane rejections: non-abort thrown ports, malformed host results, and rejecting trace sinks.
...
The thin linked artifact remains loadable through the unchanged callable `createPlaybookRuntime` default-export contract, and SLC shall keep treating the module as opaque rather than inspecting its factory internals [[3]].
...
`requiredRoleIds` stay the source-declared player names verbatim, keeping the documented `--player <Player>=…` bindings true to the source [[8]].
...
The demo reference sets shall be recompiled against the adopted set: the English reference immediately, through the documented `slc playbook workflow.txt` → `playbook run ./workflow.ts` flow including the documented role flags [[8]], and the Chinese reference by the maintainer from the released packages.
...
- Cased English sources run end to end under Playbook's published `playbook run` host, with `--player` flags matching the names the README documents [[8]]; caseless sources keep their exact prior behavior.
...
- [[phase-execution-30](../packages/phase-execution.md#phase-execution-30)]'s stated provenance set matches the configured selector again.
...
[4]: https://github.com/sublang-ai/playbook/blob/v2.0.0/src/runtime.ts "Playbook 2.0.0 runtime contract"
[5]: https://github.com/sublang-ai/playbook/blob/v2.0.0/slc/text2gears.md#players "Playbook 2.0.0 player naming contract"
[6]: https://github.com/sublang-ai/playbook/blob/v2.0.0/slc/link.md#player-binding "Playbook 2.0.0 linked player binding"
[7]: https://github.com/sublang-ai/playbook/blob/v2.0.0/reference/sdlc/code.playbook/bin/run.js "Playbook 2.0.0 run host role binding"
[8]: https://github.com/sublang-ai/playbook/blob/v2.0.0/README.md#run-a-playbook-non-interactively "Playbook 2.0.0 non-interactive run documentation"
