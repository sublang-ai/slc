...
Deliver the `playbook` domain pipeline end to end so `slc playbook <source>` compiles a workflow description into a runnable `playbook` artifact and performs that compilation through compiled meta-phase artifacts, verified against the manual reference at `../playbook/reference/sdlc/code.playbook/`.
`slc playbook` is the generic `slc <pipeline> <source>` invocation ([DR-001](../decisions/001-slc-pipeline-layout-naming-invocation.md#cli)) with the `playbook` pipeline (`text2gears` → `gears2fsm` → `link`), not a new CLI verb; the reference `code.playbook/` directory is exactly its [DR-001](../decisions/001-slc-pipeline-layout-naming-invocation.md#output-locations) artifact directory.
...
- [ ] `slc playbook <source>` and `slc playbook <source> --link <target>` producing `<basename>.playbook/{<basename>.gears.md, <basename>.fsm.ts, <basename>.playbook.ts}` under interpreted execution, with an integration test over a faked agent transport (extends `COMPILE`, `SELFHOST`)
...
4. **Run `slc playbook <source>` interpreted end to end.**
   Drive `slc playbook code.md` (compile chain) and `slc playbook code.md --link <target>` through interpreted execution to `code.playbook/{code.gears.md, code.fsm.ts, code.playbook.ts}` at their [DR-001](../decisions/001-slc-pipeline-layout-naming-invocation.md#output-locations) locations.
