...
- A compiled phase reaches agents, judges, status, and telemetry only through `PlaybookPorts` and receives no host capability beyond them; its agents write only the `target` or `linked` path, and the [DR-003](../decisions/003-slc-phase-execution.md) generic checks enforce that write scope as for interpreted execution.
