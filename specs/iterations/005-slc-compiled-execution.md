...
- [x] The `slc` meta-pipeline definitions — `text2gears`, `gears2fsm` (auditable GEARS-to-FSM mapping), and the reserved `link` — consumed from `@sublang/playbook`'s `slc/` and confirmed to chain and infer, rather than duplicated here (per [DR-005](../decisions/005-slc-self-hosting-meta-pipeline.md)'s Playbook-owned source; Boss-approved option 2)
