...
A post-close review (reviewer: GPT-5.6 Sol) converged on two foundational corrections, consolidated into one hardening commit over the eight-commit rewrite, then completed by seven follow-up commits.
...
The follow-ups closed the review's remaining gaps on the same two foundations: publication became end-to-end identity-gated — at most one build per orderly run and none when nothing recordable survived, every published step output re-read through the safe reader against the identity captured at acceptance while the source embeds its pre-executor capture, protected-path changes detected structurally and propagated so a failed executor's mutation also kills the carried record for its target — and the plan's read/write inventory was completed with the host-written entry and verification files, the reserved `.slc`/`.slc-verify` directories, the pin index, and every pin-selected locator, with containment decided lexically on canonical paths before directories are created.
...
The fifth follow-up unified the remaining path and observation primitives: one prospective physical relation decides every planned-write conflict — targets, host files, and created directories pairwise, entry planning included — so absent paths reached through aliased parents can no longer coincide; and reuse selection observes the recorded target once and atomically, its bytes becoming the recorded identity, with an unobservable target failing the run instead of being silently skipped.
The sixth follow-up made the reservation and the relation global and typed: `.slc` and `.slc-verify` are refused as write destinations anywhere — a foreign bundle's marker can no longer be resurrected through `-o` or a direct link; a planned write must resolve to a possible location (its nearest existing anchor a directory) and no planned file may contain another planned write; an absent protected path's prospective location cannot be squatted by a target; and DR-021 now states per-copy proportional degradation at the decision level.
The seventh follow-up made the sink relation typed and total at both boundaries: the reserved-namespace ban moved into `runPhase` itself, so a directly driven phase is refused a `.slc` destination like any planned one; strict prospective resolution distinguishes genuine absence from a dangling symbolic link, turning an untyped mid-run directory failure into a typed plan refusal; a planned directory whose place a file holds refuses the plan instead of failing at mkdir; and the verifier-support closure is declared as managed writes preflighted with the plan, so a blocked `.slc-verify` root surfaces before the agent pipeline, not after it.
